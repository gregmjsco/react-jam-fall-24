...
export default class TutorialGame extends Phaser.Scene {
        ...
        Rune.initClient({
            onChange: ({ game }) => {
                ...
                for (const body of physics.allBodies(game.world)) {
                    ...
                    if (body.data?.player) {
                        const controls =
                            game.controls[body.data?.playerId ?? ""];
                        if (controls) {
                            if (controls.left) {
    ...
    update() {
        ...
        if (this.cursors) {
            const stateLeft =
                (this.cursors?.left.isDown ?? false) || gameInputs.left;
            const stateRight =
                (this.cursors?.right.isDown ?? false) || gameInputs.right;
